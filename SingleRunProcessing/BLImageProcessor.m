function [red_centroids,red_velocity,red_velocity_s,red_velocity_r,red_R2,red_g2SNR,red_signal,...
    uncertainty_wall_loc_pix,numimages] = BLImageProcessor(run,filepath,image_start_end,...
    ...
    near_wall_extrap,rotation_angles,total_sig_thresh,cfd_turb_prof,imageprocess_numbers)
...
red_move_ROI_ud = []; %correction in pixels
    ...
    parfor imloop = 1:length(imageprocess_numbers)
        ...
        %Load image
            ...
            imageData = imrotate(imageData,rotation_angles(run),'bilinear','crop');
            imageData_ROI = imageData(ROI(1):ROI(2),ROI(3):ROI(4)); %get ROI
            end
    %filtering
    total_roi_sig = sum(imageData_ROI(:));
    if total_roi_sig<total_sig_thresh(run)
        ...
        %Process image
        [temp_centroids,temp_velocity,temp_velocity_r,temp_velocity_s,temp_R2,...
            temp_snr,temp_signal] = BLImageCurveFitter_VelocityCalculator(imageData_ROI,cols,...
            ...
            constant_background,run,cfd_turb_prof,synth_switch);
        ...
        red_R2 = [red_R2,temp_R2];                      %R2 as f(y) for each image
        red_g2SNR = [red_g2SNR,temp_snr];      %SNR as f(y) for each image
        red_signal = [red_signal,temp_signal];
    ...
    WaitMessage.Destroy
    uncertainty_wall_loc_pix = 2*std(red_move_ROI_ud);
