function [ideal_synth_image,nondim_velo_error,synth_input_tau_fit,synth_input_velocity_mean,...
         ROI_mean_SNR,ROI_gate1_location_bounds,ROI_gate2_location_bounds,Amp_g2_mod] = SynthData_Ideal(rows,...
...
%no velocity fluctuations
    ...
    %% Fitting information as f(y)
            Amp_g1 = doublegauss_fitvariables(:,1)/Gates(1); 
            Width_g1 = doublegauss_fitvariables(:,3);    %pixel width
            C1 = round(emissionlocatingdata(1))-max(dx_real1)+dx_real1;        %pixel location
                       
            Amp_g2 = doublegauss_fitvariables(:,4)/Gates(2);
    ...
    %% Estimating the error in the mean velocity as a result of intensity decay
        [ideal_velocity_error,ideal_pixel_error,nondim_velo_error] = SynthData_VelocityErrorCalculator(gate2_trimmed,gate1_trimmed,gate2_image_ROI,...
