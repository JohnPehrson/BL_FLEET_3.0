...
%% Initialization of variables
 %get runs to use
    ...
    DAQ_P0 = cell(length(uniqueruns),1);
    ...
    Run_P0 = zeros(length(uniqueruns),2); %left column is mean, right column is 2 std
    Run_rho = zeros(length(uniqueruns),2); %left column is mean, right column is 2 std
    Run_T_inf = zeros(length(uniqueruns),1); %left column is mean, right column is 2 std
    Run_P_inf = zeros(length(uniqueruns),1); %left column is mean, right column is 2 std
    ...
    %% Load in Reynolds numbers and time from each run
        ...
        ACE_generalfilepath = 'D:/ACEDAQ';
            ...
            for k = 1:length(used_filepaths)
                ...
                single_DAQ_T_inf = readmatrix(full_filepath,'Sheet','Reduced Data','Range','AU2:AU1000');
                single_DAQ_P0 = readmatrix(full_filepath,'Sheet','Reduced Data','Range','B2:B1000');
                single_DAQ_P_inf = readmatrix(full_filepath,'Sheet','Reduced Data','Range','AT2:AT1000');
                ...
                DAQ_T_inf{k} = single_DAQ_T_inf;
                DAQ_P0{k} = single_DAQ_P0;
                DAQ_P_inf{k} = single_DAQ_P_inf;
                DAQ_rho{k} = single_DAQ_rho;
            end

            save(savename_DAQ,'DAQ_times','DAQ_Res','DAQ_Machs','DAQ_Velos','DAQ_T0','DAQ_P0','DAQ_rho','DAQ_P_inf','DAQ_T_inf');
...
for i = 1:length(uniqueruns)
    ...
    single_DAQ_P0   = DAQ_P0{i};
    single_DAQ_rho = DAQ_rho{i};
    single_DAQ_P_inf   = DAQ_P_inf{i};
    single_DAQ_T_inf = DAQ_T_inf{i};
    ...
    mean_P0 = mean(single_DAQ_P0(binary_run));
    mean_rho = mean(single_DAQ_rho(binary_run));
    mean_P_inf = mean(single_DAQ_P_inf(binary_run));
    mean_T_inf = mean(single_DAQ_T_inf(binary_run));
    ...
    doublestd_P0 = 2*std(single_DAQ_P0(binary_run));
    ...
    Run_P0(i,:) = [mean_P0,doublestd_P0];
    Run_rho(i,:) = [mean_rho,doublestd_rho];
    Run_T_inf(i,:) = mean_T_inf;
    Run_P_inf(i,:) = mean_P_inf;
end


Run_Re_scaled = Run_Re/(10^6);
Run_P0 = Run_P0.*6.89476;
Run_P_inf = Run_P_inf./1000;


save("ACEConditions.mat",'Run_Re_scaled','Run_Mach','Run_Mean_Velo','Run_T0','Run_P0','Run_rho','Run_T_inf','Run_P_inf');
